...
for i, (title, slots, lru_index) in enumerate(frames, start=1):
    ...
    # MRU -> LRU label
    svg_elements.append(f'<text x="{start_x + (slot_w*1.5 + gap)}" y="{start_y + slot_h + 38}" font-family="Helvetica, Arial, sans-serif" font-size="16" text-anchor="middle" fill="#111">MRU → LRU (left → right)</text>')
    ...
    arrow_top_y = start_y - 30
    arrow_mid_y = start_y - 6
    ...
    svg_elements.append(f'<text x="{lx}" y="{arrow_top_y - 8}" font-family="Helvetica, Arial, sans-serif" font-size="14" text-anchor="middle" fill="#111">LRU</text>')
